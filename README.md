...
**Ollama not responding inside Docker**

On Linux, `host.docker.internal` may not resolve. Get your host IP:
```bash
ip route | grep default | awk '{print $3}'
# use that IP in OLLAMA_URL, e.g. http://172.17.0.1:11434
```

**MongoDB connection refused**

Make sure Docker Compose is fully up: `docker compose ps`. MongoDB takes ~5s to initialize on first run.

**Files not loading after restart**

Uploads are stored in a named Docker volume `uploads_data`. As long as you don't run `docker compose down -v`, files persist across restarts.

**LLaVA vision not working**

Check if the model is pulled: `ollama list`. If `llava` is not there, run `ollama pull llava`. The app falls back to OCR automatically.

**Redis connection errors in logs**

Redis is optional — the app works without it, falling back to MongoDB for conversation history. The warning `Redis error (will use DB fallback)` is non-fatal.